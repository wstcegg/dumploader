...
int CMiniDumpReader::Open(CString sFileName, CString sSymSearchPath)
{
    ...
    if(m_hFileMiniDump==INVALID_HANDLE_VALUE)
    {
		...
		//printf("create file failed, error code: %d", err);
        Close();
        return 1;
    }
...
int CMiniDumpReader::StackWalk_exception()
{
	if (NULL == m_DumpData.m_pExceptionThreadContext)
		return INT_MAX;

	// Make modifiable context
	//CONTEXT Context;
	//memcpy(&Context, &m_DumpData.m_pExceptionThreadContext, sizeof(CONTEXT));
	g_pMiniDumpReader = this;
	...
	case PROCESSOR_ARCHITECTURE_INTEL:
		dwMachineType = IMAGE_FILE_MACHINE_I386;
		sf.AddrPC.Offset = m_DumpData.m_pExceptionThreadContext->Eip;
		sf.AddrStack.Offset = m_DumpData.m_pExceptionThreadContext->Esp;
		sf.AddrFrame.Offset = m_DumpData.m_pExceptionThreadContext->Ebp;
		break;
#endif
#ifdef _AMD64_
	case PROCESSOR_ARCHITECTURE_AMD64:
		dwMachineType = IMAGE_FILE_MACHINE_AMD64;
		sf.AddrPC.Offset = m_DumpData.m_pExceptionThreadContext->Rip;
		sf.AddrStack.Offset = m_DumpData.m_pExceptionThreadContext->Rsp;
		sf.AddrFrame.Offset = m_DumpData.m_pExceptionThreadContext->Rbp;
		break;
#endif
#ifdef _IA64_
	case PROCESSOR_ARCHITECTURE_AMD64:
		dwMachineType = IMAGE_FILE_MACHINE_IA64;
		sf.AddrPC.Offset = m_DumpData.m_pExceptionThreadContext->StIIP;
		sf.AddrStack.Offset = m_DumpData.m_pExceptionThreadContext->IntSp;
		sf.AddrFrame.Offset = m_DumpData.m_pExceptionThreadContext->RsBSP;
		sf.AddrBStore.Offset = m_DumpData.m_pExceptionThreadContext->RsBSP;
		break;
#endif
	default:
	{
	...
	for (;;)
	{
		BOOL bWalk = ::StackWalk64(
			dwMachineType,               // machine type
			m_DumpData.m_hProcess,       // our process handle
			(HANDLE)m_DumpData.m_uExceptionThreadId,          // thread ID
			&sf,                         // stack frame
			dwMachineType == IMAGE_FILE_MACHINE_I386 ? NULL : m_DumpData.m_pExceptionThreadContext, // used for non-I386 machines
			ReadProcessMemoryProc64,     // our routine
			FunctionTableAccessProc64,   // our routine
			GetModuleBaseProc64,         // our routine
			NULL                         // safe to be NULL
		...
		m_DumpData.m_CrashStack.push_back(stack_frame);
	}

	//CString sStackTrace;
	//UINT i;
	//for (i = 0; i < m_DumpData.m_Threads[m_DumpData.m_uExceptionThreadId].m_StackTrace.size(); i++)
	//{
	//	MdmpStackFrame& frame = m_DumpData.m_Threads[m_DumpData.m_uExceptionThreadId].m_StackTrace[i];
	//	if (frame.m_sSymbolName.IsEmpty())
	//		continue;
	//	CString sModuleName;
	//	CString sAddrPCOffset;
	...
	//		CString number;
	//		number.Format(_T("%02x"), md5_hash[i]);
	//		m_DumpData.m_Threads[m_DumpData.m_uExceptionThreadId].m_sStackTraceMD5 += number;
	//	}
	//}
	//m_DumpData.m_Threads[m_DumpData.m_uExceptionThreadId].m_bStackWalk = TRUE;
